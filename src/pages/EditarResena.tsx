...
export default function EditarResena() {
    ...
    const botonEditarResena = async (e: React.FormEvent) => {
        ...
        try {
            const resenasEditadas: EditarResena = {
                id: datosResena.id,
                comentario: datosResena.comentario,
                numeroEstrellas: datosResena.numeroEstrellas
            };
            const respuesta = await editarResena(id, resenasEditadas)
                            ...
                            <Grid container spacing={3} sx={{ justifyContent: 'center' }}>
                                <Grid size={{ xs: 12, md: 6 }}>
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                                        ...
                                        <TextField required fullWidth label="Comentario de Opinión" variant="outlined" value={datosResena.comentario} multiline rows={5} onChange={editarComentario} sx={{ bgcolor: '#fff', borderRadius: 3, '& .MuiOutlinedInput-root': { borderRadius: 3, fontWeight: 600 } }} />
